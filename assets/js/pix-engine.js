/**
 * PIX Engine - Gerador de BR Code (Padrão Banco Central do Brasil)
 * e Renderizador de QR Code nativo sem dependências externas.
 */
...
class PixEngine {
  ...
  /**
   * Calcula o Checksum CRC16 (Polinômio 0x1021 - CCITT-FALSE)
   */
  static calculateCRC16(payload) {
    let crc = 0xFFFF;
  ...
  /**
   * Normaliza textos retirando acentos para conformidade com o padrão EMV
   */
...
/**
 * QRCode Generator ultra-leve nativo (Algoritmo Reed-Solomon / Matrix)
 * Inspirado na implementação pública de Kazuhiko Arase
 */
(function (global) {
  ...
  QRCodeModel.prototype = {
    ...
    make: function () {
      if (this.typeNumber < 1) {
        for (let t = 1; t < 40; t++) {
          let rsBlocks = QRRSBlock.getRSBlocks(t, this.errorCorrectLevel);
          ...
          for (let i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(data.getLength(), QRUtil.getLengthInBits(data.mode, t));
            data.write(buffer);
          }
  ...
  QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
    ...
    let totalDataCount = 0;
    for (let i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
    if (buffer.getLengthInBits() > totalDataCount * 8) throw new Error("QR Overflow");
  ...
  QRCodeModel.createBytes = function (buffer, rsBlocks) {
    ...
    let dcdata = new Array(rsBlocks.length), ecdata = new Array(rsBlocks.length);
  ...
  let QRUtil = {
    ...
    getLengthInBits: function (mode, type) {
      if (1 <= type && type < 10) {
        return (mode == 1) ? 10 : (mode == 2) ? 9 : (mode == 4) ? 8 : 8;
      } else if (type < 27) {
  ...
  // Renderizador SVG/Canvas
  global.generateQRCodeCanvas = function (text, canvas, size = 260) {
    let qr = new QRCodeModel(0, 0); // Type 0 (auto-detect), Error correction L
    qr.addData(text);
    qr.make();
    ...
    let tileW = size / count;
    let tileH = size / count;
